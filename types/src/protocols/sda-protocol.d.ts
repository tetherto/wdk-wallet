...
export type SdaQuote = {
    ...
    expiry?: number;
    /**
     * - The provider quote identifier, if the provider issues one.
     */
    id?: string;
};
/**
...
export type SdaCreateOptions = {
    ...
    /**
     * - Request a reusable address, for providers that let the caller pick reusable vs single-use per request.
     */
    reusable?: boolean;
};
/**
...
export type SdaDepositAddress = {
    ...
    refundAddress?: string;
    /**
     * - Unix timestamp (seconds) at which the address's activation expires, for a provider whose activation model ({@link SdaActivationModel}) is `'ttl'`.
     */
    expiry?: number;
};
/**
